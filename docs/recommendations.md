...
10. Re-enable auth on `/process-routes/*` and `/optimization-logs`; wire the mobile login/signup to the backend. — Partly: `/optimization-logs` requires a JWT and register/login work again (passlib was broken with bcrypt 5). `/process-routes/*` waits for the frontend login flow.
