...
13. Restrict the Google Maps key by referrer / package name. — **Obsolete.** The map now uses MapLibre with OpenFreeMap tiles, so there is no Maps key; disable the old key in Google Cloud.
