"""
vroom_solver.py  –  VROOM (pyvroom) solver, run in-process.

pyvroom 1.14 needs numpy<2; requirements.txt pins that for the whole service.
On Windows, vroom_matrix_patch works around an MSVC buffer-format quirk in the
wheel; elsewhere importing it is a no-op.
"""
...
from . import vroom_matrix_patch  # noqa: F401  (must be imported before building vroom.Input)

logger = logging.getLogger(__name__)

# Objective weights VROOM optimizes with. Differs from the scorer's metadata weights (finding B4).
W1_COST = 0.7
W2_TIME = 0.3
...
def solve_vroom(input_data: dict, matrix_edge_list: list, file_bytes: bytes) -> dict:
    """Solve with VROOM and return the route-sequence dict solver.py scores. input_data is unused."""
    W1, W2 = W1_COST, W2_TIME
    ...
    matrix_edge_list = matrix_edge_list or []
    ...
    logger.info("[VROOM] Matrix: %d locations, %d OSRM pairs (%.0f%% coverage), rest haversine fallback",
                n_locs, osrm_hit, _pct)
